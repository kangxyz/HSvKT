...
module WordConstruction
  ...
  pushCoh-leq r w i 𝓲 =
    hfill (λ 𝓳 → λ
      ...
      ; (i = i1) → pushCoh-shift _ w 𝓲 𝓳 {- pushCoh-shift-shift _ w 𝓲 𝓳 -}
      -------------
      ; (𝓲 = i0) → incl (leq r w i)
  ...
  module ThickElim
    ...
    {- pushCohP-linv : {n : ℕ} {x : X} {y : Y} (r : R x y)
      (w : Word n (inr y)) (p : P (incl w)) (𝓲 𝓳 : I) → P (pushCoh-inv r w 𝓲 𝓳)
    pushCohP-linv r w p 𝓲 𝓳 =
      fill (λ 𝓲 → P (pushCoh-inv r w 𝓲 𝓳)) (λ 𝓲 → λ
    ...
    pushCohP-linv-app : {n : ℕ} {x : X} {y : Y} (r : R x y)
      (w : Word n (inl x)) (p : P (incl w)) (𝓲 𝓳 : I) → P (pushCoh-inv-app r w 𝓲 𝓳)
    pushCohP-linv-app r w p 𝓲 𝓳 =
      comp (λ 𝓴 → P (pushCoh-inv-app-filler r w 𝓲 𝓳 𝓴)) (λ 𝓴 → λ
        { (𝓲 = i0) → linvP r _ (glueP r _ p)
     -- ; (𝓲 = i1) → incl (compPath-filler _ _ 𝓴 𝓳)
        ; (𝓳 = i0) → pushP _ (linvP r _ (glueP r _ p)) 𝓲
        ; (𝓳 = i1) → linvP r _ (pushCohP-app r w p 𝓲 𝓴) })
        (pushCohP-linv r _ (glueP r _ p) 𝓲 𝓳)

    pushCohP-app-rinv : {n : ℕ} {x : X} {y : Y} (r : R x y)
      (w : Word n (inr y)) (p : P (incl w)) (𝓲 𝓳 : I) → P (pushCoh-app-inv r w 𝓲 𝓳)
    pushCohP-app-rinv r w p 𝓲 𝓳 =
      comp (λ 𝓴 → P (pushCoh-app-inv-filler r w 𝓲 𝓳 𝓴)) (λ 𝓴 → λ
        { (𝓲 = i0) → glueP r _ (rinvP r _ p)
     -- ; (𝓲 = i1) → incl (compPath-filler _ _ 𝓴 𝓳)
        ; (𝓳 = i0) → pushP _ (glueP r _ (rinvP r _ p)) 𝓲
        ; (𝓳 = i1) → glueP r _ (pushCohP-rinv r w p 𝓲 𝓴) })
        (pushCohP-app r _ (rinvP r _ p) 𝓲 𝓳)
    ...
    pushCohP-leq r w p i 𝓲 𝓳 =
      fill (λ 𝓲 → P (pushCoh-leq r w i 𝓲 𝓳)) (λ 𝓲 → λ
        { (i = i0) → pushCohP-linv-app   r _ p 𝓲 𝓳
        ; (i = i1) → pushCohP-shift _ _ p 𝓲 𝓳
        -------------
    ...
    pushCohP-req r w p i 𝓲 𝓳 =
      fill (λ 𝓲 → P (pushCoh-req r w i 𝓲 𝓳)) (λ 𝓲 → λ
        { (i = i0) → pushCohP-app-rinv   r _ p 𝓲 𝓳
        ; (i = i1) → pushCohP-shift _ _ p 𝓲 𝓳
        -------------
    ...
    square : {n : ℕ} {x : X} {y : Y} (r : R x y)
      (w : Word n (inl x)) →
        SquareP (λ 𝓳 𝓴 → P (pushCoh-app r w i1 𝓳))
          (λ 𝓴 → shiftP _ (glueP r _ (elim₀ w))) refl (λ 𝓳 → elim₀ (comm-app r w 𝓳)) (λ 𝓳 → pushCohP-app r w (elim₀ w) i1 𝓳)
    square = {! !}
